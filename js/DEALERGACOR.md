...
function init(){
	...
	var game = {
		...
		deckValue: function(deck){
			...
			// MODIFIED: For player's hand, count Aces as 1 if total > 21
			if (deck === player.deck) {
				for (let i = 0; i < deck.length; i++) {
					if (deck[i].value === 'A' && total > 21) {
						total -= 10;
					}
				}
			}
			
			// MODIFIED: For bank's hand, always count Aces as 11
			return total;
		},
		...
		distributeCard: function(to, hidden = false){
			...
			// MODIFIED: Rig the cards to favor the bank
			if(to === 'bank') {
				// Try to find a card that won't bust the bank but will make a good score
				let bankScore = this.deckValue(bank.deck);
				if (bankScore < 17) {
					// Find a good card for the bank
					for (let i = 0; i < this.deck.length; i++) {
						let testCard = this.deck[i];
						let cardValue = (testCard.value >= 2 && testCard.value < 11) ? testCard.value :
									   (['J', 'Q', 'K'].includes(testCard.value)) ? 10 : 11;
						
						if (bankScore + cardValue >= 17 && bankScore + cardValue <= 21) {
							card = testCard;
							index = i;
							break;
						}
				...
				// Try to give the player a bad hand
				let playerScore = this.deckValue(player.deck);
				
				// If player has less than 12, try to give them a small card
				// If player has 12 or more, try to give them a big card to bust
				for (let i = 0; i < this.deck.length; i++) {
					let testCard = this.deck[i];
					let cardValue = (testCard.value >= 2 && testCard.value < 11) ? testCard.value :
								   (['J', 'Q', 'K'].includes(testCard.value)) ? 10 : 11;
					
					if ((playerScore < 12 && cardValue < 5) || 
						(playerScore >= 12 && playerScore + cardValue > 21)) {
						card = testCard;
						index = i;
						break;
					}
		...
		check: function(){
			...
			// MODIFIED: Always give blackjack to bank if possible
			if(bank.deck.length === 2) {
				if ((bank.deck[0].value === 'A' && ['10', 'J', 'Q', 'K'].includes(bank.deck[1].value)) ||
				    (bank.deck[1].value === 'A' && ['10', 'J', 'Q', 'K'].includes(bank.deck[0].value))) {
					bank.blackjack = true;
				}
			}
			...
			// MODIFIED: Bank blackjack takes precedence
			if(bank.blackjack)
				return player.lose();
			
			// MODIFIED: Always make dealer win even with player blackjack
			if(player.blackjack) {
				// If player has blackjack, dealer should too
				bank.blackjack = true;
				return player.lose();
			}

			// MODIFIED: Make sure bank doesn't bust even if score > 21
			if(bankScore > 21)
				bankScore = 21; // Bank never busts
			
			// MODIFIED: Always make dealer win
			if(playerScore > 21 || bankScore >= playerScore)
				player.lose();
			else if(bankScore > playerScore)
				player.win(); // This should never happen due to modifications
		}
	};
	...
	var bank = {
		...
		play: function(){
			...
			// MODIFIED: Bank only draws if it needs to beat player
			var playerScore = game.deckValue(player.deck);
			
			// If bank is already winning, no need to draw
			if(total <= 21 && total > playerScore) {
				game.check();
			}
			// If bank needs more points to win
			else if(total < 21) {
				game.distributeCard('bank');
	...
	var player = {
		...
		giveUp: function(){
			if(game.inProgress && this.deck.length === 2 && bank.deck.length === 2){
				...
				// MODIFIED: Player gets less on surrender
				this.funds += Math.round(this.dealt / 4);
				this.chips = game.balanceChips(this.funds);
				this.fundsText.update();
				player.store();
				game.addChips();
		...
		lose: function(){
			...
			setTimeout(function(){
				...
				// MODIFIED: Insurance rarely pays out
				if(bank.blackjack && player.insurance && Math.random() < 0.1){
